...
//     final response = await dio.get('/api/articles'); // Gọi danh sách thay vì chi tiết
...
class ArticleRemoteDataSourceImpl implements ArticleRemoteDataSource {
  ...
  Future<List<ArticleModel>> getArticles() async {
    try {
      final response = await dio.get('/api/articles'); // Endpoint của bạn

      // ✅ SỬA Ở ĐÂY: API trả về List trực tiếp, không phải Page ['content']
  ...
  // --- 2. GIỮ NGUYÊN HÀM LẤY CHI TIẾT ---
  @override
  Future<ArticleModel> fetchArticleBySlug(String slug) async {
    try {
      // Gọi đúng API chi tiết (thêm ID/Slug vào cuối)
